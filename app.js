...
function encrypt(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    encryptedStr += String.fromCharCode(str.charCodeAt(i) + (salt.length % 5 + 1) * Math.pow(-1, i) * (i % 13));
...
function decrypt(str) {
  ...
  // if (str.length < )
  for (var i = 0; i < str.length; i++) {
    decryptedStr += String.fromCharCode(str.charCodeAt(i) - (salt.length % 5 + 1) * Math.pow(-1, i) * (i % 13));
...
// console.log(decrypt('qg`ueNyE0eu[hm'));
